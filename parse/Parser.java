...
public class Parser {
	...
	/*
	 * A classe Parser possui um procedimento para cada não terminal
	 */
	
	private Lexer lex; //analisador léxico para este analisar sintártico
	private Token look; //lookhead tagen
	Env top = null; //tabela de símbolos corrente ou do topo
	int used = 0; //memória usada oara declarações
	...
	void error (String s) {throw new Error("near line "+lex.line+": "+s);}
	 ...
	 public void program() throws IOException{ //program -> block
		 /*
		  * O procedimento chama block() para analisar sintaticamente o fluxo de entrada e
		  * construir a árvore de sintaxe. 
		  */
		 Stmt s = block();
	 ...
	 /*
	  * O procedimento block() faz o tratamento da tabela de símbolos
	  */
	 Stmt block() throws IOException{ // block -> { decls stmts }
		 match('{'); Env savedEnv = top; top = new Env(top); // top contém a tabela de símbolos do topo, (linha 12)
		 //savedEnv é um elo para a tabela de símbolos anterior.
		 decls(); Stmt s = stmts();
		 match('}'); top = savedEnv;
		 return s;		 
	 }
	 
	 /* As declarações resultam em entradas da tabela de símbolos para identificadores(linha 61)*/
	 ...
	 /*
	  * O procedimento stmt possui um comando switch coom cases correspondendo 
	 ...
	 /*
	  * O procedimento assign é auxiliar para as atribuições
	  * */
	 ...
	 /*
	  * Análise sintática das expressôes aritméticas e boolianas
	  * */
	 ...
	 /*
	  * Tratamento de fatores de expressões.
	  * O procedimento auxiliar offset gera código para cálculos de
	  * endereço de arranjo.
	  * */
	 ...
	 Access offset(Id a) throws IOException{ //I-> [E] | [E] I
		 ...
		 Type type = a.type;
		 match('['); i = bool(); match(']'); //primeiro indice, I -> [E]
		 type = ((Array)type).of;
